...
const colorChangedBack = function(){
    sayHelloButton.style.backgroundColor='#EFEFEF'
    sayHelloButton.style.color='black'
}

sayHelloButton.addEventListener('mouseout', colorChangedBack)
...
const ohMy = () => {
    ...
    .then((response) => {
        ...
        for(let i = 0; i < response.data.length; i++){
            ...
            newElementP.textContent = response.data[i]
...
const repeatMyParam = () => {
   ...
   axios.get(`http://localhost:3000/repeat/jared`)
   .then((response) => {
    console.log(response.data)
...
function newQuery (){
    ...
    axios.get('http://localhost:3000/query-test/?name=oliver&age=33')
    .then((response) => {
        console.log(response.data)
